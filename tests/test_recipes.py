...
class TestRecipesTestCase(BaseTestCase):
    def test_creating_recipes(self):
        with self.client:
            ...
            response = self.client.post('/recipes/1', headers = h, data = self.recipe)
    ...
    def test_duplicate_recipe(self):
        ...
        with self.client:
            ...
            h.add('x-access-token', auth)
            category = self.client.post('/category', headers = h, data = self.category)
            recipe = self.client.post('/recipes/1', headers = h, data = self.recipe)
            response = self.client.post('/recipes/1', headers = h, data = self.recipe)
            result = json.loads(response.data)
    ...
    def test_empty_fields(self):
        ...
        with self.client:
            ...
            h = Headers()
            h.add('x-access-token', auth)
            response = self.client.post('/recipes/1', headers = h, data = data)
            result = json.loads(response.data)
    ...
    def test_get_all(self):
        with self.client:
            ...
            h.add('x-access-token', auth)
            category = self.client.post('/category', headers = h, data = self.category)
            responses = self.client.post('/recipes/1', headers = h, data = self.recipe)
    ...
    def test_edit_recipe(self):
        ...
        with self.client:
            ...
            category = self.client.post('/category', headers = h, data = self.category)
            recipe = self.client.post('/recipes/1', headers = h, data = self.recipe)
            response = self.client.put('/recipes/1', headers = h, data = data)
            result = json.loads(response.data)
            self.assertEqual(response.status_code, 200)
    ...
    def test_edit_none_existantrecipe(self):
        ...
        with self.client:
            ...
            category = self.client.post('/category', headers = h, data = self.category)
            recipe = self.client.post('/recipes/1', headers = h, data = self.recipe)
            response = self.client.put('/recipes/3', headers = h, data = data)
            result = json.loads(response.data)
